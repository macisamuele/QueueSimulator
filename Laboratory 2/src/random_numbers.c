...
#define UPTOMOD -2836
#define RATIO  (1.0/MODULE)           /* 1/MODULE */
...
/*
 **  Function: double uniform(double a,  double b)
...
/*
 **  Function: double negexp(double mean)
...
/*
 **  Function: double pareto(double alpha, double kappa)
...
/*
 **  Function: double pareto_m(double alpha, double kappa)
...
double pareto_m(double alpha, double mean) {
	...
	double kappa;
	if (alpha <= 1.0) {
		fprintf(stderr, "Error pareto_m(): alpha must be greater than 1.0\n");
		return (mean);
	}
	rnd32();
	u = (seed) * RATIO;
...
/*
 **  Function: int poisson(double alpha)
...
/*
 **  Function: int geometric0(double mean)
...
/*
 **  Function: int geometric1(double mean)
 **  Return  : a random value distributed geometrically with average 'mean',
...
/*
 **  Function: int geometric_trunc1(double mean, int max_len)
 **  Return  : a random value distributed geometrically with average 'mean',
 **            starting from 1.
...
int geometric_trunc1(double mean, int max_len) {
	...
	if (fabs(old_mean - mean) > 1e-5 || old_max != max_len) {
		...
		do {
			...
			df_p = (mean - len) * status_N + len * status_N * temp_res / status
					- mean;
...
/*
 **  Function: int trunc_exp(double mean, long length)
